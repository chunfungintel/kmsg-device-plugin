...
func newDevicePlugin(sysfsDir, devfsDir, kmsgDev string, options cliOptions) *devicePlugin {
	dp := &devicePlugin{
		kmsgDev:          kmsgDev,
		sysfsDir:         sysfsDir,
		devfsDir:         devfsDir,
...
func (dp *devicePlugin) scan() (dpapi.DeviceTree, error) {
	...
	for _, f := range files {
		...
		if !dp.isCompatibleDevice(f.Name()) {
			klog.V(4).Info("Non-Compatible: ", f.Name())
			continue
		}
...
func main() {
	...
	plugin := newDevicePlugin(sysfsDrmDirectory, devfsDriDirectory, kmsgDev, opts)
